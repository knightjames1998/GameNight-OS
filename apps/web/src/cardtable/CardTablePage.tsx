...
import { SESSION_PACKS, CARD_TABLE_CONFIG } from "@gamenight/shared";
...
// The Card Table page: its shell, its header, its brand, and then the shared
// title-night screens. Everything below the header is in
// ../titlenight/TitleNight.tsx, which Board Game draws too.
//
...
const PACK = SESSION_PACKS.cardtable;
...
export default function CardTablePage() {
  ...
  const { ctx, session, loading, busy, err, call, startSession } =
    usePackSession<TnSessionView, TnCtx>({
      ...
      eventId,
      replacePrompt:
        "A session is already in progress on this event. Replace it? Every hand recorded in the current session stays in your stats, but the session itself is ended.",
    });

  if (!eventId) {
    return <div className="tn-root ct-root"><div className="tn-wrap"><p className="tn-hint">No event specified.</p><BackButton /></div></div>;
  }
  if (loading) {
    return <div className="tn-root ct-root"><div className="tn-wrap"><p className="tn-hint">Loading...</p></div></div>;
  }

  return (
    <div className="tn-root ct-root">
      <div className="tn-wrap">
        ...
        <div>
          <div className="tn-brand">Card <em>Table</em></div>
          <div className="tn-sub">Deal, tap the finish order, done</div>
        </div>
