...
function LivePlay({
  ...
  const setChar = (playerId: string, character: string | null) =>
    call(`/api/marioparty/${eventId}/character`, { playerId, character });
...
function RecordBoard({
  ...
  const toggleBonus = (pid: string, star: string) =>
    setBonus((s) => {
      const cur = s[pid] ?? [];
      return { ...s, [pid]: cur.includes(star) ? cur.filter((x) => x !== star) : [...cur, star] };
    });
  ...
  const record = () => {
    onRecord({
      ...
      lines: active.map((p) => ({ playerId: p.id, stars: starNum(p.id), bonusStars: bonus[p.id] ?? [] })),
    });
    setStars({});
    setBonus({});
    setWinnerId(null);
  ...
  return (
    <div className="mp-card">
      ...
      <div className="mp-lab" style={{ marginTop: 14 }}>Stars &amp; bonus</div>
      {session.roster.map((p) => (
        <div key={p.id} style={{ padding: "8px 0", borderTop: "1px solid #33244f" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <input type="checkbox" checked={!!inGame[p.id]} onChange={() => setInGame((s) => ({ ...s, [p.id]: !s[p.id] }))} />
            <div style={{ flex: 1 }}>
          ...
          {inGame[p.id] && bonusOptions.length > 0 && (
            <div className="mp-bonus">
              {bonusOptions.map((star) => (
                <button
                  key={star}
                  className={(bonus[p.id] ?? []).includes(star) ? "on" : ""}
                  onClick={() => toggleBonus(p.id, star)}
                >
                  {star}
                </button>
              ))}
