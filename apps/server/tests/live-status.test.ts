...
import { deriveLiveState, STALE_AFTER_MS, STALE_CHECK_MS } from "../../web/src/livestatus.js";
...
test("no socket at all is DOWN, whatever the clock says", () => {
  // The ordinary state of Home and Login, which is why the pill renders nothing
  // for it: a screen with no live subscription is not a screen with a problem.
  assert.equal(deriveLiveState({ openCount: 0, lastMessageAt: NOW, now: NOW }), "down");
  assert.equal(deriveLiveState({ openCount: 0, lastMessageAt: 0, now: NOW }), "down");
  ...
  assert.equal(deriveLiveState({ openCount: -1, lastMessageAt: NOW, now: NOW }), "down");
});

test("an open socket that heard something recently is LIVE", () => {
  assert.equal(deriveLiveState({ openCount: 1, lastMessageAt: NOW, now: NOW }), "live");
  assert.equal(deriveLiveState({ openCount: 1, lastMessageAt: at(1_000), now: NOW }), "live");
  ...
  assert.equal(deriveLiveState({ openCount: 3, lastMessageAt: at(5_000), now: NOW }), "live");
});

test("an open socket that has gone quiet past the window is STALE", () => {
  assert.equal(
    deriveLiveState({ openCount: 1, lastMessageAt: at(STALE_AFTER_MS + 1), now: NOW }),
    "stale",
  );
  assert.equal(deriveLiveState({ openCount: 1, lastMessageAt: at(120_000), now: NOW }), "stale");
});
...
test("OPEN AND NEVER HEARD ANYTHING IS STALE, not a special case", () => {
  ...
  assert.equal(deriveLiveState({ openCount: 1, lastMessageAt: 0, now: NOW }), "stale");
});

test("the boundary is inclusive, so the window is not off by a tick", () => {
  assert.equal(
    deriveLiveState({ openCount: 1, lastMessageAt: at(STALE_AFTER_MS), now: NOW }),
    "live",
...
test("a clock that jumps backwards does not invent a stale screen", () => {
  ...
  assert.equal(deriveLiveState({ openCount: 1, lastMessageAt: NOW + 10_000, now: NOW }), "live");
});
...
test("THE STALE WINDOW SURVIVES ONE DROPPED PING AND NOT TWO", () => {
  ...
  assert.equal(
    deriveLiveState({ openCount: 1, lastMessageAt: at(oneMissed), now: NOW }),
    "live",
  ...
  assert.equal(
    deriveLiveState({ openCount: 1, lastMessageAt: at(twoMissed), now: NOW }),
    "stale",
