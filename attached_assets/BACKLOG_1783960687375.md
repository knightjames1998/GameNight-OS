...
- [ ] Standalone generalized bracket (quick play with typed names, no event). Deferred this round: brackets are group/event-scoped for stats identity, so quick play needs its own design. Beerio Kart already runs standalone.
...
## STANDING RULES
- Every game mode/tracker gets a back-to-event button somewhere (Beerio: the corner Event chip when launched from an event).
