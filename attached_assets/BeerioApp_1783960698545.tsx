...
export default function App(){
  ...
  // GameNight prefill: fires once per EVENT per device (tracked in
  // localStorage), hosts only. Refreshing mid-tournament never clobbers;
  // launching a different event always prefills fresh over old state.
  useEffect(()=>{
    if(!GN_EVENT||isSpectator)return;
    try{if(localStorage.getItem("gamenight-beerio-event-v1")===GN_EVENT)return;}catch{/* still prefill */}
    fetch(`${API}/beerio-context/${GN_EVENT}`)
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(!d||!Array.isArray(d.prefill)||d.prefill.length<2)return;
        try{localStorage.setItem("gamenight-beerio-event-v1",GN_EVENT);}catch{/* prefill anyway */}
