...
// ─── Racer colors ─────────────────────────────────────────────────────────────
// 16 distinct kart colors, all dark enough to carry white text.
const PALETTE = [
  "#E5484D","#F76B15","#E6A700","#46A758","#12A594","#0090FF","#6E56CF","#E93D82",
  "#8E4EC6","#00749E","#299764","#AD7F58","#5B5BD6","#DC3E42","#7E808A","#978365",
];
...
function normalizeColors(raw: unknown, n: number): string[] {
  ...
  const fresh=PALETTE.filter(c=>!used.has(c));
  const out:string[]=[];
  for(let i=0;i<n;i++) out.push(base[i]||fresh.shift()||PALETTE[i%PALETTE.length]);
  return out;
}
                ...
                {heat.map(seed=>{
                  ...
                  return(
                    <button key={seed} disabled={readOnly||assigned} onClick={()=>tap(seed)} style={{touchAction:"manipulation"}}
                      ...
                      <span className="w-3 h-3 rounded-full flex-shrink-0 border border-[var(--ink)]" style={{background:colorOf(seed)}}/>
            ...
            {standings.map(r=>{
              ...
              return(
                <div key={r.seed} className={`flex items-center gap-2 border-2 border-[var(--ink)] rounded-[9px] px-2.5 py-1.5 ${leader?"bg-[var(--sun)] shadow-[0_2px_0_rgba(22,35,59,.18)]":"bg-white"}`}>
                  ...
                  <span className="w-3 h-3 rounded-full flex-shrink-0 border border-[var(--ink)]" style={{background:colorOf(r.seed)}}/>
        ...
        {open&&(
          <div className="px-4 py-3 flex flex-wrap gap-5 items-start">
            <div className="flex-1 min-w-[270px]">
                      ...
                      <div key={it.key} className="bg-white border-2 border-[var(--ink)] rounded-[11px] p-2.5">
                        ...
                        <div className={`grid gap-1.5 ${it.options.length>2?"grid-cols-2":"grid-cols-2"}`}>
                          {it.options.map(o=>{
                            ...
                            return(
                              <button key={o.v} disabled={!canPick||!named} onClick={()=>canPick&&named&&onPick(it.key,o.v)} style={{touchAction:"manipulation"}}
                                className={`relative overflow-hidden text-left px-2.5 py-2 rounded-[9px] border-2 border-[var(--ink)] transition-all ${picked?"bg-[var(--sun)] shadow-[0_2px_0_rgba(22,35,59,.22)]":"bg-[#FBF8F0]"} ${canPick&&named?"cursor-pointer hover:brightness-[1.03] active:translate-y-px":"cursor-default"}`}>
                                <span className="absolute left-0 top-0 bottom-0 opacity-20 pointer-events-none" style={{width:`${pct}%`,background:o.color,transition:"width .4s ease"}}/>
                                <span className="relative flex items-center gap-1.5">
                                  <span className="w-2.5 h-2.5 rounded-full flex-shrink-0 border border-[var(--ink)]" style={{background:o.color}}/>
                                  <span className="font-[Fredoka] font-bold text-[12px] text-[var(--ink)] flex-1 min-w-0 truncate">{o.name}</span>
                                  {n>0&&<span className="font-[Nunito] font-bold text-[10px] text-[var(--ink-soft)] flex-shrink-0">{pct}%</span>}
                                  {picked&&<span className="text-[10px] flex-shrink-0">✔</span>}
                                </span>
                              </button>
                            );
                          })}
              ...
              {canPick&&named&&items.length>0&&<p className="font-[Nunito] text-[10.5px] font-semibold text-[var(--muted)] mt-2 mb-0 leading-snug">Tap to call the winner. You can change a pick until the result is recorded.</p>}
            </div>
...
export default function App(){
  ...
  const handleColorChange=useCallback((i:number,c:string)=>{
    setColors(prev=>{const a=normalizeColors(prev,Math.max(prev.length,i+1));a[i]=c;return a;});
    setPickerFor(null);
  },[]);
  ...
  useEffect(()=>{
    ...
    const key=`b|${champName}|${new Date().toISOString().slice(0,10)}|${realCount}p|${done}h`;
  ...
  useEffect(()=>{
    ...
    const key=`g|${champName}|${new Date().toISOString().slice(0,10)}|${realCount}p|${gpLog.length}h`;
    try{if(localStorage.getItem(HOF_FLAG_PREFIX+key))return;localStorage.setItem(HOF_FLAG_PREFIX+key,"1");}catch{/* still dedup by key */}
    const stats:Record<string,HofStat>={};
                ...
                {names.map((name,i)=>(
                  <div key={i} className="relative">
                    <button type="button" title="Tap to change kart color" onClick={()=>setPickerFor(pickerFor===i?null:i)} style={{touchAction:"manipulation",background:colorOf(i),color:textOn(colorOf(i))}}
                      className="absolute left-2 top-1/2 -translate-y-1/2 font-[Fredoka] font-bold text-[10.5px] border border-[var(--ink)] w-[17px] h-[17px] rounded-[4px] grid place-items-center z-10 cursor-pointer">{i+1}</button>
                    ...
                    {pickerFor===i&&(
                      <div className="absolute left-0 top-full mt-1 z-30 bg-white border-2 border-[var(--ink)] rounded-[10px] p-2 shadow-[0_4px_0_rgba(22,35,59,.22)] grid grid-cols-8 gap-1.5 w-max">
                        {PALETTE.map(c=>(
                          <button key={c} type="button" onClick={()=>handleColorChange(i,c)} style={{background:c,touchAction:"manipulation"}}
                            className={`w-6 h-6 rounded-[6px] border-2 cursor-pointer ${colorOf(i)===c?"border-[var(--ink)] outline outline-2 outline-[var(--sun)]":"border-[var(--ink)] opacity-90 hover:opacity-100"}`}/>
                        ))}
                      </div>
                    )}
                  </div>
