...
const GN_EVENT: string | null =
  typeof location !== "undefined" ? new URLSearchParams(location.search).get("event") : null;

function reportToGameNight(key: string, placements: { name: string; place: number }[]): void {
  if (!GN_EVENT || placements.length === 0) return;
  fetch(`${API}/beerio-complete`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ eventId: GN_EVENT, key, placements }),
...
export default function App(){
  ...
  // GameNight prefill: only when launched from an event, only for hosts, and
  // only while nothing has been recorded (a tournament in progress is never
  // clobbered; setup-screen names from an older night are).
  const gnPrefilled=useRef(false);
  useEffect(()=>{
    if(!GN_EVENT||isSpectator||gnPrefilled.current)return;
    if(Object.keys(results).length>0||gpLog.length>0)return;
    gnPrefilled.current=true;
    fetch(`${API}/beerio-context/${GN_EVENT}`)
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(!d||!Array.isArray(d.prefill)||d.prefill.length<2)return;
